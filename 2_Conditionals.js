...
function exercise1(num1) {
  ...
  if (num1 > 10) { //Seemed exclusive based on wording "greater than 10"
...
function exercise2(num2) {
  ...
  // --------------------------------------------
  //Assigned remainer to variable to make code easier to read
  let remainder = num2%2; 
  if (remainder === 0) { //Used === to avoid other data types like '0'
    answer2 = num2 + " is even";
  } else if (remainder !== 0) {//Will return odd if remainder is not 0
    answer2 = num2 + " is odd";
  }
  // --------------------------------------------
  // And above here
...
function exercise3(num3) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  if (num3 >= 0) { //Inclusive because 0 is positive
    answer3 = num3 + " is positive";
  } else if (num3 < 0) { //Exclusive because only <0 is negative
...
function exercise4(varA, varB) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  //Used == to ensure strings and numbers are still considered to be equal
  if (varA == varB) { 
...
// One example would be if I were to compare a string to a number. Since the operands are being compared with a strictly equal sign, they would need to be the same data type to be equal. If varA was a string "5" and varB was 5, the answer would be that they differ because they are different data types.
...
function exercise6(varA, varB, varC) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  //Both conditions separated by && must be true for the output to be true
  //Only varA !== varC is in the condition because varA has to equal varB to be true, so it is not necessary to also compare varB to varC
...
function exercise8(amount1, amount2, minimum, maximum) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  //Used paranthesis to group together the amounts and stop the computer from reading left to right
  //Both amount1 and amount2 must be less than minimum to be true, and same for the comparison to maximum
  //Used && to prevent left to right reading that may turn a true/false into a 0 or 1
  if (minimum < (amount1 && amount2) && maximum > (amount1 && amount2)){
    answer8 = true;
  } else {
    answer8 = false;
  }
...
console.log(exercise8(8,4,2,9));
console.log(exercise8(8,4,10,12));
//In the first example, 8 and 4 are greater than the minimum of 2, and are both less than the maximum of 9. The output is true.
//In the second example, 8 and 4 are not less than the minimum of 10, despite being less than the maximum of 12, the output is false
...
function exercise9(item) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  //Used typeof to compare the data type of the input to the string "number". If the data type is not a number, the if statement will reassign answer9 to the string.
  if (typeof item !== "number") {
    answer9 = "Please send a number, that was a " + typeof item;
  //Used elseif to create an alternative output in case the prior condition was a number and not a different data type. If the input is strictly a number (not a string) and one of the cases, the switch statement will go through the cases and output the corresponding string. If it is a different number that was not assigned a string, the default will print "Thanks for that!"
...
function exercise10(num10) {
  ...
  // Write your code for the exercise below here:
  // --------------------------------------------
  //Used modulo on the input to check if num10 is divisible by the specified numbers. Used an if statement to assign strings for each situation.
  //Used === because the question did not specify whether we should account for different data types.
  if (num10%15 === 0) {
      answer10 = "Fizz Buzz";
    } else if (num10%5 === 0) {
      answer10 = "Buzz";
...
//I found these exercises to be medium, because I needed to think a little harder to understand which operators to use. However, it was not too bad.
